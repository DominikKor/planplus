...
def teacher(request, term: str):
    periods = Period.objects.filter(teacher__contains=term).order_by("number")
...
def room(request, term: str):
    periods = Period.objects.filter(room__contains=term).order_by("number")
    periods = get_unique(periods)
    return render(request, "plan/plan.html", {"source": "Räume", "plans": [periods], "table_head": term})


def class_(request, term: str):
    periods = Period.objects.filter(plan__cls__contains=term).order_by("number")
    periods = get_unique(periods)
